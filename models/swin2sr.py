...
class RSTB(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        result = self.patch_embed(self.conv(self.patch_unembed(self.residual_group(x, x_size), x_size)))
...
class Swin2SR_DFE(nn.Module):
    ...
    def __init__(self, img_size=(64, 64), patch_size=1,
                 embed_dim=96, depths=[6, 6, 6, 6,], num_heads=[6, 6, 6, 6],
                 ...
                 **kwargs):
        super(Swin2SR_DFE, self).__init__()
        self.img_range = img_range

        self.window_size = window_size

        self.gradient_chechpointing = use_checkpoint
    ...
    def forward_features(self, x, ):
        ...
        for layer in self.layers:
            if self.gradient_chechpointing and self.training:
